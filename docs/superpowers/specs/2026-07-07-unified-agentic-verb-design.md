...
3. **Anthropic deferred.** Two witnesses validate neutrality. *(Corrected 2026-07-08 via the maintained claude-api reference — the pre-registered "recheck when it does" ran: Anthropic DOES now have a stateful-agentic surface, **Managed Agents** (`/v1/agents` + `/v1/sessions`, beta `managed-agents-2026-04-01`). But it is a **hosted agent-session + SSE-event** paradigm where the SERVER runs the tool loop — NOT a Responses/Interactions-shaped stateful **generation call** where the client runs tools. Wrong shape for `respond()`; it is a future Layer-C surface, sibling to Gemini Live / OpenAI Realtime, and gets its own spec. The Messages API remains stateless — no `previous_response_id`-style chaining.)* Folding it in here would either shoehorn a hosted-session model into a single-call verb or duplicate the chat verb.
